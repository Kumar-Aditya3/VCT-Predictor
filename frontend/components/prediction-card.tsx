    ...
    >
      ...
      <div className="mt-4 rounded-xl border border-line bg-surface2/40 p-3">
        ...
        <div className="mt-2 grid gap-2 md:grid-cols-2">
          {prediction.player_projections.map((player) => (
            <div key={player.player_name} className="flex items-center justify-between rounded-lg bg-black/20 px-3 py-2">
              <div>
              ...
              <p className="font-semibold">
                {player.projected_kills.toFixed(1)} / {player.projected_deaths.toFixed(1)}
              </p>
            </div>
          ))}
        </div>
