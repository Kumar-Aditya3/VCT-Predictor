...
def validation_report_from_bundle(model_bundle: dict | None, generated_at: str, source: str) -> ValidationReportResponse:
    ...
    return ValidationReportResponse(
        ...
        experiment_id=str(model_bundle.get("experiment", {}).get("id")) if model_bundle.get("experiment", {}).get("id") else None,
        dataset_start=model_bundle.get("dataset_snapshot", {}).get("date_range", {}).get("start"),
        dataset_end=model_bundle.get("dataset_snapshot", {}).get("date_range", {}).get("end"),
        selection_metric=model_bundle.get("experiment", {}).get("selection_metric", {}),
